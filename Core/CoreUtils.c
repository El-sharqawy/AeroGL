...
bool MakeDirectory(const char* fullPath)
{
	...
	if (result != 0)
	{
		...
		strerror_s(errorBuf, sizeof(errorBuf), errorCode);
#else
		// Linux Version
		char* msg = strerror(errorCode);
		strncpy(errorBuf, msg, sizeof(errorBuf) - 1);
		errorBuf[sizeof(errorBuf) - 1] = '\0'; // Manually ensure null-termination
#endif
		syserr("Error Creating Folder: %s with Code: %d", errorBuf, errorCode);
		return (false);
	}
