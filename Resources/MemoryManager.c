...
typedef struct AERO_ALIGN(16) SMemoryBlockHeader
{
	size_t size;
...
void MemoryManager_PrintData()
{
	...
	else
	{
		...
		syslog("Allocation Count: %zu", psMemoryManager->allocationCount);
...
#else
	(void)file; // Explicitly cast to void to silence the warning
	(void)line;
#endif
