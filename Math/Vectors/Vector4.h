 ...
 */
typedef struct AERO_ALIGN(16) SVector4f
{
	union
	{
		struct
...
static inline Vector4 Vector4_Divs(const Vector4 vec1, float scalar)
{
	...
	Vector4 res = { vec1.x * inv, vec1.y * inv, vec1.z * inv, vec1.w * inv };
	return res;
}
...
{
	Vector4 result = { a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t, a.z + (b.z - a.z) * t, a.w + (b.w - a.w) * t };
	return result;
}
