...
static inline Vector3 Vector3Zero() {
	Vector3 v;
	v.reg = _mm_setzero_ps(); // Faster than setr_ps(0,0,0,0)
	return v;
}

static inline Vector3 Vector3One() {
	Vector3 v;
	v.reg = _mm_setr_ps(1.0f, 1.0f, 1.0f, 0.0f);
	return v;
}
...
{
	Vector3 result = { vec1.x - scalar, vec1.y - scalar, vec1.z - scalar };
	return (result);
}
...
static inline Vector3 Vector3_Divs(const Vector3 vec1, float scalar)
{
	...
	Vector3 result = { vec1.x * inv, vec1.y * inv, vec1.z * inv };
	return (result);
}
...
{
	Vector3 result = { a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t, a.z + (b.z - a.z) * t };
	return (result);
}

static inline Vector3 Vector3_Negate(const Vector3 vec3)
{
	Vector3 result = { -vec3.x, -vec3.y, -vec3.z };
	return (result);
}

static inline Vector3 Vector3_Random()
{
	Vector3 result = { random_float(), random_float(), random_float() };
	return (result);
}

static inline Vector3 Vector3_RandomRange(float min, float max)
{
	Vector3 result = { random_float_range(min, max), random_float_range(min, max), random_float_range(min, max) };
	return (result);
}
